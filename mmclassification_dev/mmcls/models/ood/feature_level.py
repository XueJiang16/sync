...
class FeatureMapSim(BaseModule):
    ...
    def forward(self, **input):
        ...
        if self.has_ood_detector:
            ...
            # ood_scores *= patch_sim
            ood_scores, _ = self.ood_detector(**input)
            patch_sim[0] = ((1 / 0.5) ** (self.order)) * torch.pow(patch_sim[0], self.order)
            patch_sim[0][patch_sim[0] > 1] = 1
            ood_scores *= patch_sim[0]
            ood_scores, _ = self.ood_detector(**input)
            patch_sim[1] = ((1 / 0.4) ** (self.order)) * torch.pow(patch_sim[1], self.order)
            patch_sim[1][patch_sim[1] > 1] = 1
            ood_scores *= patch_sim[1]
        else:
            ood_scores = patch_sim
        return ood_scores, type
