...
class AugContrast(BaseModule):
    ...
    def forward(self, **input):
        if "type" in input:
            ...
            del input['type']
        with torch.no_grad():
            outputs_orig, features_orig = self.classifier(return_loss=False, softmax=True, post_process=False,
                                                          require_features=True, **input)
            input["img"] = input["img"] + (torch.rand_like(input["img"])-0.5) / 4
            outputs_aug, features_aug = self.classifier(return_loss=True, softmax=True, post_process=False,
                                                        require_features=False, **input)
